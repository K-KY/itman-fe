...
const API_URL = 'http://localhost:8080/states';
...
async function getAllStates(pageRequest: PageRequest, group:number): Promise<{
    totalPages: number;
    content: SimpleBoard[]
}> {
    try {
...
async function getStates(pageRequest: PageRequest, group:number|null): Promise<{
    totalPages: number;
    content: SimpleBoard[]
}> {
    try {
...
async function postState(name: string, description: string, group:number|null): Promise<SimpleBoard|null> {
    try {
...
async function patchState(seq: number, name: string, description: string,  group:number|null): Promise<SimpleBoard|void> {
    try {
...
export {getAllStates, getStates, postState, patchState, patchEnable, getCount, getCountAll};
